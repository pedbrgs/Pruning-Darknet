...
if __name__ == '__main__':
    ...
    parser.add_argument('--pruning-rate', type = float, help = 'pruning rate (0 to 0.99)')
    ...
    save_weights(model, path = 'pruned_' + opt.weights)
    version = int(opt.network.split('v')[-1])    
    model_to_cfg(model, cfg = 'pruned_' + opt.cfg, mode = 'train', version = version)